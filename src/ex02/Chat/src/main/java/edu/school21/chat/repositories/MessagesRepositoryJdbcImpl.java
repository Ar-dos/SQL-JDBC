...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void save(Message message) {
        ...
        ResultSet resultSet = null;
        try {
            pstmt = connection.prepareStatement("SELECT * FROM users where id = ? AND login = ? AND password = ?");
            ...
            pstmt = connection.prepareStatement("SELECT * FROM chatrooms inner join users_chatrooms on room_id = Chatrooms.id where id = ? AND name = ? AND user_id = ?");
            ...
            pstmt = connection.prepareStatement("INSERT INTO messages (author_id, chatroom_id, text, date_time) VALUES (  ? , ?, ?, ?)", PreparedStatement.RETURN_GENERATED_KEYS);
            pstmt.setLong(1, message.getAuthor().getId());
